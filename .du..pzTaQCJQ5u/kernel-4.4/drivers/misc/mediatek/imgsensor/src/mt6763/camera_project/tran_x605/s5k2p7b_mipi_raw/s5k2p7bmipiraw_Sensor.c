...
#include "s5k2p7bmipiraw_Sensor.h"

#define PFX "S5K2P7B_camera_sensor"
#define LOG_INF(format, args...)	pr_debug(PFX "[%s] " format, __FUNCTION__, ##args)

static DEFINE_SPINLOCK(imgsensor_drv_lock);
//add XLLSHLSS-403 long exp by ming.liao 20180120 start
static kal_uint32 shutter_choose = 0;
#define M_SHUTTER_0_5S 		54686
#define M_SHUTTER_1S 		109373
#define M_SHUTTER_1_5S 	    164059
#define M_SHUTTER_2S 		218746
#define M_SHUTTER_4S 		437493
#define M_SHUTTER_6S 		656239
...
write_cmos_sensor(0x0202, 0xD59F);
write_cmos_sensor(0x0340, 0xD5A4);
write_cmos_sensor(0X0700, 0x0000);
write_cmos_sensor(0x0702, 0X0000);
		break;
		case M_SHUTTER_1S: //101502
		LOG_INF("1s shutter =%d, framelength =%d, realtime_fps =%d\n", shutter,imgsensor.frame_length, realtime_fps);
write_cmos_sensor(0x0202, 0x06AC);
write_cmos_sensor(0x0340, 0x06B1);
write_cmos_sensor(0X0700, 0x0006);
write_cmos_sensor(0x0702, 0X0606);
		break;
		case M_SHUTTER_1_5S: //152253
		LOG_INF("1.5s shutter =%d, framelength =%d, realtime_fps =%d\n", shutter,imgsensor.frame_length, realtime_fps);
write_cmos_sensor(0x0202, 0x0A03);
write_cmos_sensor(0x0340, 0x0A08);
write_cmos_sensor(0X0700, 0x0006);
write_cmos_sensor(0x0702, 0X0606);
		break;
		case M_SHUTTER_2S: //203004
		LOG_INF("2s shutter =%d, framelength =%d, realtime_fps =%d\n", shutter,imgsensor.frame_length, realtime_fps);
write_cmos_sensor(0x0202, 0x0D59);
write_cmos_sensor(0x0340, 0x0D5E);
write_cmos_sensor(0X0700, 0x0006);
write_cmos_sensor(0x0702, 0X0606);
		break;
		case M_SHUTTER_4S: //406008
		LOG_INF("4s shutter =%d, framelength =%d, realtime_fps =%d\n", shutter,imgsensor.frame_length, realtime_fps);
write_cmos_sensor(0x0202, 0x1AB3);
write_cmos_sensor(0x0340, 0x1AB8);
write_cmos_sensor(0X0700, 0x0006);
write_cmos_sensor(0x0702, 0X0606);
		break;
		case M_SHUTTER_6S: //609013
		LOG_INF("6s shutter =%d, framelength =%d, realtime_fps =%d\n", shutter,imgsensor.frame_length, realtime_fps);
write_cmos_sensor(0x0202, 0x280D);
write_cmos_sensor(0x0340, 0x2812);
write_cmos_sensor(0X0700, 0x0006);
write_cmos_sensor(0x0702, 0X0606);
		break;
		default:
...
static void preview_setting(void)
{
    ...
    write_cmos_sensor_8(0x0100,0x00);
    check_output_stream_off();//modify for check output stream on and off by wei.miao@reallytek.com 20180122
...
static void hs_video_setting(void)
{
  ...
  write_cmos_sensor(0xf440,0x402F);
	...
	write_cmos_sensor(0x300A,0x0001);
	write_cmos_sensor(0x0200,0x0200);
	write_cmos_sensor(0x0202,0x0384);
	write_cmos_sensor(0x0204,0x0020);
	write_cmos_sensor(0x37C0,0x0002);
	write_cmos_sensor(0x37C2,0x0103);
	write_cmos_sensor(0x3004,0x0003);
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_SET_HDR:
			spin_lock(&imgsensor_drv_lock);
			imgsensor.ihdr_mode = *((UINT8*) feature_data);
			spin_unlock(&imgsensor_drv_lock);
...
UINT32 S5K2P7B_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
