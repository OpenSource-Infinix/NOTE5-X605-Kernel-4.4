...
#include "../../drivers/misc/mediatek/sensors-1.0/gyroscope/inc/cust_gyro.h"
#include "../../drivers/misc/mediatek/sensors-1.0/gyroscope/inc/gyroscope.h"
#include "../../drivers/misc/mediatek/sensors-1.0/accelerometer/icm206xx/icm206xx_register_20608D.h"
#include "../../drivers/misc/mediatek/sensors-1.0/accelerometer/icm206xx/icm206xx_share.h"
...
static ssize_t store_chip_orientation(struct device_driver *ddri, const char *buf, size_t tCount)
{
        ...
        if (res == 0) {
                if (hwmsen_get_convert(nDirection, &obj->cvt) != 0)
                       GYRO_PR_ERR("ERR: fail to set direction\n");
                else
                       obj->hw.direction = nDirection;
        }
...
static int icm206xx_gyro_enable_nodata(int en)
{
        ...
        if (1 == en)
        {
                ...
                power_gyro = false;
                res = icm206xx_share_EnableSensor(ICM206XX_SENSOR_TYPE_GYRO, power_gyro);
                icm206xx_share_SetSampleRate(ICM206XX_SENSOR_TYPE_GYRO, 2000000, true);
                res = icm206xx_share_SetPowerMode(ICM206XX_SENSOR_TYPE_GYRO, power_gyro);
        }
        if (res != ICM206XX_SUCCESS) {
...
static int icm206xx_gyro_i2c_remove(struct i2c_client *client)
{
        ...
        //res = misc_deregister(&icm206xx_gyro_devicue);
        if (res)
