...
//modify XLLSHLSS-1778 by ming.liao 20180418 start
char *envp[2];
//modify XLLSHLSS-1778 by ming.liao 20180418 end
static int main_opening = 0;
static int sub_opening = 0; 
...
static int aw3644_flash_write(struct i2c_client *client, u8 reg, u8 val)
{
	...
	while( mLoopCount < 3)
	{
		...
		if (ret < 0)
		{
			...
			msleep(3);
			continue;
			pr_debug("wei.wang2 debug failed writing at 0x%02x\n", reg);
		}
		else
		{
			mLoopCount = 0;
	...
	if(mLoopCount >= 3)
	{
		pr_err("[wei.wang2debug][Camera]aw3644 writing failed,mLoopCount retry > 3!!\n");
	}
	return ret;
}
...
//torch (mA) = (Code * 2.91) + 2.55 (mA)//197mA 104mA
static const unsigned char aw3644_torch_level[AW3644_LEVEL_NUM] = {
	0x23, 0x43, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
static const unsigned char aw3644_flash_level[AW3644_LEVEL_NUM] = {
	0x15, 0x20, 0x2b, 0x36, 0x41, 0x4e, 0x5A, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
static int aw3644_enable(int channel)//wwsub
{
	...
	aw3644_verity_level();	
	msleep(1);
	if (channel == AW3644_CHANNEL_CH0) //warm,led2
	{
	    printk("aw3644_enable main_duty=%d\n",main_duty);
	    if(main_duty < 2)
	    {
		    aw3644_flash_write(AW3644_i2c_client, AW3644_REG_TORCH_LEVEL_LED1,0x00);//clean bit 0-7 to 0
			msleep(3);
		    aw3644_flash_write(AW3644_i2c_client, AW3644_REG_TORCH_LEVEL_LED2,aw3644_torch_level[main_duty]&0x7F);//bit 1~6
		    msleep(3);
		    aw3644_flash_write(AW3644_i2c_client, AW3644_REG_ENABLE,0x0a );//torch mode ,enable led
			msleep(3);
			main_opening = 1; //main torch
		    printk("aw3644_enable CH0 torch mode done\n");
	    }
	    else
	    {
			printk("aw3644_enable CH0 flash mode done main_duty start:%d\n",main_duty);
		    aw3644_flash_write(AW3644_i2c_client, AW3644_REG_FLASH_LEVEL_LED1,0x00);//clean bit 0-7 to 0
			msleep(3);
		    aw3644_flash_write(AW3644_i2c_client, AW3644_REG_FLASH_LEVEL_LED2,aw3644_flash_level[main_duty]&0x7F);
		    msleep(3);
		    aw3644_flash_write(AW3644_i2c_client, AW3644_REG_ENABLE,0x0e );//flash mode ,enable led
			msleep(3);
			main_opening = 2;//main flash
		    printk("aw3644_enable CH0 flash mode done\n");
	    }
	}
	else if(channel == AW3644_CHANNEL_CH1) //cold,led1
	{
	    printk("aw3644_enable sub_duty=%d\n",sub_duty);
	    if(sub_duty < 1)
	    {
			if(main_opening == 1) //main torch,sub torch
			{
				aw3644_flash_write(AW3644_i2c_client, AW3644_REG_TORCH_LEVEL_LED1,aw3644_torch_level[sub_duty]&0x7F);//clean bit 7 to 0 ,then led2 not be override
				msleep(3);
				aw3644_flash_write(AW3644_i2c_client, AW3644_REG_ENABLE,0x0b );//torch mode ,enable led
				sub_opening = 1; //main torch,sub torch
			}
			else if(main_opening == 2)//main flash,sub torch
			{
				...
				msleep(3);
				aw3644_flash_write(AW3644_i2c_client, AW3644_REG_ENABLE,0x0f );//flash mode ,enable led
				sub_opening = 2; //main flash,sub flash
			}
			else //sub torch
			{
				aw3644_flash_write(AW3644_i2c_client, AW3644_REG_TORCH_LEVEL_LED1,aw3644_torch_level[sub_duty]&0x7F);//clean bit 7 to 0 ,then led2 not be override
				msleep(3);
				aw3644_flash_write(AW3644_i2c_client, AW3644_REG_ENABLE,0x09 );//torch mode ,enable led
				sub_opening = 3; //sub torch
			}
			msleep(3);
		    printk("aw3644_enable CH1 torch mode done\n");
	    }
	    else
	    {
			if(main_opening == 1) //main torch,sub flash
			{
				aw3644_flash_write(AW3644_i2c_client, AW3644_REG_FLASH_LEVEL_LED2,aw3644_flash_level[main_duty]&0x7F);//bit 1~6
				msleep(3);
				aw3644_flash_write(AW3644_i2c_client, AW3644_REG_FLASH_LEVEL_LED1,aw3644_flash_level[sub_duty]&0x7F);//clean bit 7 to 0 ,then led2 not be override
				msleep(3);
				aw3644_flash_write(AW3644_i2c_client, AW3644_REG_ENABLE,0x0f );//torch mode ,enable led
				sub_opening = 2; //main flash,sub flash
			}
			else if(main_opening == 2)//main flash,sub flash
			{
				aw3644_flash_write(AW3644_i2c_client, AW3644_REG_FLASH_LEVEL_LED2,aw3644_flash_level[main_duty]&0x7F);//bit 1~6
				msleep(3);
				aw3644_flash_write(AW3644_i2c_client, AW3644_REG_FLASH_LEVEL_LED1,aw3644_flash_level[sub_duty]&0x7F);//clean bit 7 to 0 ,then led2 not be override
			...
			{
				aw3644_flash_write(AW3644_i2c_client, AW3644_REG_FLASH_LEVEL_LED2,aw3644_flash_level[sub_duty]&0x7F);//clean bit 7 to 0 ,then led2 not be override
				msleep(3);
				aw3644_flash_write(AW3644_i2c_client, AW3644_REG_ENABLE,0x0d );//flash mode ,enable led
				sub_opening = 4; //sub flash
			}
		    printk("aw3644_enable CH1 flash mode done\n");
			msleep(3);
	    }		
		main_opening = 0;
	}
...
{
	 printk("aw3644_disable_main sub_opening:%d\n",sub_opening);
	 if(sub_opening == 1) //main torch,sub torch
	 {
		aw3644_flash_write(AW3644_i2c_client,AW3644_REG_ENABLE,0x09);
	 }
	 else if(sub_opening == 2) //main flash,sub flash
	 {
		aw3644_flash_write(AW3644_i2c_client,AW3644_REG_ENABLE,0x0d);
	 }
	 else if(sub_opening == 3) //sub torch
	 {
		//aw3644_flash_write(AW3644_i2c_client,AW3644_REG_ENABLE,0x08);
	 }
	 else if(sub_opening == 4) //sub flash
	 {
		//aw3644_flash_write(AW3644_i2c_client,AW3644_REG_ENABLE,0x0c);
	 }
	 else
	 {
		 aw3644_flash_write(AW3644_i2c_client,AW3644_REG_ENABLE,0x00);
	 }
	 printk("aw3644_disable_main X\n");
     return 0;
}

static int aw3644_disable_sub(void)
{
	printk("aw3644_disable_sub main_opening:%d\n",main_opening);
     if(sub_opening == 1) //main torch,sub torch
	 {
		aw3644_flash_write(AW3644_i2c_client,AW3644_REG_ENABLE,0x0a);
	 }
	 else if(sub_opening == 2) //main flash,sub flash
	 {
		aw3644_flash_write(AW3644_i2c_client,AW3644_REG_ENABLE,0x0e);
	 }
	 else if(sub_opening == 3) //sub torch
	 {
		aw3644_flash_write(AW3644_i2c_client,AW3644_REG_ENABLE,0x08);
	 ...
	 else if(main_opening == 1)  //main torch,if flash main only
	 {
		 //aw3644_flash_write(AW3644_i2c_client,AW3644_REG_ENABLE,0x00);
	 }
	 else if(main_opening == 2) //main flash,if flash main only
	 {
		 //aw3644_flash_write(AW3644_i2c_client,AW3644_REG_ENABLE,0x00);
	 }
	 else
...
//9: flash mode
	unsigned int  mode = 0;
	const int torch[2] = {0x3A,0x4A};  //torch (mA) = (Code * 2.91) + 2.55 (mA)  // 125*2
	...
	if((mode>0)&&(mode<=2))
	{
		...
		aw3644_flash_write(AW3644_i2c_client, AW3644_REG_TORCH_LEVEL_LED1,torch[mode-1]&0x7F);//bit 1~6
	...
	else if(mode == 10)
	{
		...
		aw3644_pinctrl_set(AW3644_PINCTRL_PIN_HWEN, AW3644_PINCTRL_PINSTATE_HIGH);
		mdelay(3);
		aw3644_flash_write(AW3644_i2c_client, AW3644_REG_TORCH_LEVEL_LED1,0x0D &0x7F);//clean bit 7 to 0 ,then led2 not be override
		//aw3644_flash_write(AW3644_i2c_client, AW3644_REG_TORCH_LEVEL_LED2,torch[mode-1]&0x7F);//bit 1~6
		aw3644_flash_write(AW3644_i2c_client, AW3644_REG_ENABLE,0x09 );//torch mode ,enable led1
	}
	else if(mode == 9)
	{
	    //modify for IN2 flashlight by RLK_camera_flash_aw3644 20171214 start
		aw3644_flash_write(AW3644_i2c_client, AW3644_REG_FLASH_LEVEL_LED1,0x54);//1000ma clean bit 7 to 0 ,then led2 not be override
		//modify for IN2 flashlight by RLK_camera_flash_aw3644 20171214 end
