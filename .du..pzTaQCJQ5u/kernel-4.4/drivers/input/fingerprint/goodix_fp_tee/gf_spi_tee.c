...
#else
        input_report_key(gf_dev->input, KEY_F11, gf_key.value);
        input_sync(gf_dev->input);
#endif
                break;
...
#if defined(SUPPORT_REE_OSWEGO) || defined(GF_BEANPOD_COMPATIBLE)
...
//      gf_debug(INFO_LOG, "%s, Sensor type : %s.\n", __func__, CONFIG_GOODIX_SENSOR_TYPE);
        ...
        if (ret != 1) {
                ...
                gf_debug(INFO_LOG, "get_fp_spi_enable ret=%d\n", ret);
                return -1;
        }
        gf_hw_reset(gf_dev, 0);
        gf_spi_clk_enable(gf_dev, 1);

        if (gf_check_9p_chip(gf_dev)  !=  0) {
                gf_debug(INFO_LOG, "gf probe read chip id is failed\n");
                gf_spi_clk_enable(gf_dev, 0);
                return -1;
        }

        set_fp_vendor(FP_VENDOR_GOODIX);
        gf_debug(INFO_LOG, "goodix set_fp_vendor done.");
        gf_spi_clk_enable(gf_dev, 0);
#endif
