...
static unsigned int ps_setting[5] = {0};
static unsigned int als_setting[1] = {0};
...
struct platform_device *alspsPltFmDeva;
/* For alsp driver get cust info */
struct alsps_hw *get_cust_alspsa(void)
{
    return &alsps_cust;
}
/*----------------------------------------------------------------------------*/
...
static int stk3x1x_enable_ps(struct i2c_client *client, int enable, int validate_reg)
{
    ...
    if(enable)
    {
        ...
        if(!(old & STK_STATE_EN_ALS_MASK))
                ...
                err = stk3x1x_master_send(client, obj->addr.intmode, &data, 1);
                if(err < 0)
                {
                        APS_LOG("stk3x1x_enable_ps() set interrupt error");
                }
         }
        else
        {
                ...
                if(err < 0)
                {
                      APS_LOG("stk3x1x_disenable_ps() set interrupt error");
                }
    }
    ...
    if(enable)
    {
        ...
        APS_LOG("%s: set enable_HT=%d, enable_LT=%d,\n", __func__,atomic_read(&obj->ps_high_thd_val),atomic_read(&obj->ps_low_thd_val));
#ifdef STK_TUNE0
    #ifdef CTTRACKING
        ...
        else
        {
            ...
            //  err = (obj->ps >= atomic_read(&obj->ps_high_thd_val))?0:1;
                ...
                {
                    ;//stk_ps_report(obj, err); //xing.fang
                }
...
//  obj->hw = get_cust_alsps_hw();
    ...
    atomic_set(&obj->state_val, 0);
    atomic_set(&obj->psctrl_val, ps_setting[0]); //0x31
    atomic_set(&obj->alsctrl_val, als_setting[0]);
    obj->ledctrl_val = ps_setting[4];//0XFF
    obj->wait_val = 0x04; //0xF; xing.fang custom
    obj->int_val = 0;
    obj->first_boot = true;
    obj->als_correct_factor = 1000;
#ifdef CTTRACKING
...
#ifdef STK_TUNE0
    obj->crosstalk = 0;
    obj->stk_max_min_diff = ps_setting[1];
    obj->stk_lt_n_ct = ps_setting[2];
    obj->stk_ht_n_ct = ps_setting[3];
    #ifdef CALI_EVERY_TIME
    obj->ps_high_thd_boot = obj->hw->ps_threshold_high;
    obj->ps_low_thd_boot = obj->hw->ps_threshold_low;
    if(obj->ps_high_thd_boot <= 0)
