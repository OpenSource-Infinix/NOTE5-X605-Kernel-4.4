		...
		if (IspInfo.DebugMask & ISP_DBG_INT) {
			...
			if (FrameStatus[module] != CAM_FST_DROP_FRAME) {
				/*IRQ_LOG_KEEPER(module, m_CurrentPPB, _LOG_INF,
				       ...
				       (sof_count[module] - 1) : (sof_count[module]),
				       (unsigned int)(fbc_ctrl1[0].Raw), (unsigned int)(fbc_ctrl2[0].Raw),
				       (unsigned int)(fbc_ctrl1[1].Raw), (unsigned int)(fbc_ctrl2[1].Raw));*/ //modify XLLWHLSE-4 reduce kernel log by ming.liao 20180514
			}
		}
...
*/ //modify XLLWHLSE-4 reduce kernel log by ming.liao 20180514
...
{
#if 1
	//IRQ_LOG_PRINTER(ISP_IRQ_TYPE_INT_CAM_A_ST, m_CurrentPPB, _LOG_INF); //modify XLLWHLSE-4 reduce kernel log by ming.liao 20180514
#else
...
static void ISP_BH_Workqueue(struct work_struct *pWork)
{
	...
	//IRQ_LOG_PRINTER(pWorkTable->module, m_CurrentPPB, _LOG_INF); //modify XLLWHLSE-4 reduce kernel log by ming.liao 20180514
}
#endif
