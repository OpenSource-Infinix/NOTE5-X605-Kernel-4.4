...
//add XLLSHLSS-5 by zhipeng.pan 20180228 start
#include <linux/of_gpio.h>
#if defined(CONFIG_MTK_EIC)
extern int mt_gpio_set_debounce(unsigned int gpio, unsigned int debounce);
#endif
//add XLLSHLSS-5 by zhipeng.pan 20180228 end
...
static void musb_host_work(struct work_struct *data)
{
	...
	if (host_mode) {
		...
		//add XLLSHLSS-5 by zhipeng.pan 20180328 start
		USBPHY_SET8(0x18, 0xF0);
		DBG(0, "Set Host Disconnect Threshold to 700mV.\n");
		//add XLLSHLSS-5 by zhipeng.pan 20180328 end
	} else {
...
//add XLLSHLSS-5 by zhipeng.pan 20180228 start
	...
	#endif
		DBG(0,"[%s] debounce: %d-%d \n", __func__, ints[0], ints[1]);
	}else{
		DBG(0,"%s: no debounce config in otg_iddig\n", __func__);
	}
#endif
//add XLLSHLSS-5 by zhipeng.pan 20180228 end
