...
//25mA to 400mA in step 12.5mA,reg 0x74
//modify torch current by ming.liao 20180103 start
static const unsigned char mt6370_torch_level[MT6370_LEVEL_TORCH] = {
	0x0c, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09,
	0x0a, 0x0b, 0x0c, 0x0c, 0x0c, 0x0c
};
//modify torch current by ming.liao 20180103 start
//modify flash by ming.liao 20171220 start
//50mA to 1500mA in step 12.5mA,reg 0x74
static const unsigned char mt6370_strobe_level[MT6370_LEVEL_FLASH] = {
	0x0b, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x0E, 0x10,
	0x12, 0x14, 0x16, 0x18, 0x1A, 0x1C, 0x44, 0x46, 0x48, 0x4A,
	0x4C, 0x4E, 0x50, 0x51, 0x52, 0x53, 0x54, 0x55, 0x56, 0x57,
	0x58, 0x5C
};
//modify flash by ming.liao 20171220 end
...
static int mt6370_set_level_ch1(int level)
{
	...
	if(mt6370_level_ch1 == 28)
	{
		flashlight_set_torch_brightness(
				flashlight_dev_ch1, mt6370_torch_level[2]);
		flashlight_set_torch_brightness(
				flashlight_dev_ch2, mt6370_torch_level[2]);
	}
	else if(mt6370_level_ch1 == 29)
	{
		flashlight_set_torch_brightness(
				flashlight_dev_ch1, mt6370_torch_level[6]);
		flashlight_set_torch_brightness(
				flashlight_dev_ch2, mt6370_torch_level[6]);

	}
	else if(mt6370_level_ch1 == 30)
	{
		flashlight_set_torch_brightness(
				flashlight_dev_ch1, mt6370_torch_level[12]);
		flashlight_set_torch_brightness(
				flashlight_dev_ch2, mt6370_torch_level[12]);
	}
	else
	{
...
static int mt6370_set_level_ch2(int level)
{
	...
	{
		flashlight_set_torch_brightness(
				flashlight_dev_ch1, mt6370_torch_level[5]);
		flashlight_set_torch_brightness(
				flashlight_dev_ch2, mt6370_torch_level[5]);
	}
	else if(mt6370_level_ch1 == 29)
	{
		flashlight_set_torch_brightness(
				flashlight_dev_ch1, mt6370_torch_level[8]);
		flashlight_set_torch_brightness(
				flashlight_dev_ch2, mt6370_torch_level[8]);

	}
	else if(mt6370_level_ch1 == 30)
	{
		flashlight_set_torch_brightness(
				flashlight_dev_ch1, mt6370_torch_level[14]);
		flashlight_set_torch_brightness(
				flashlight_dev_ch2, mt6370_torch_level[14]);
	}
	else
	{
		flashlight_set_strobe_brightness(
...
static int mt6370_operate(int channel, int enable)
{
	...
	if (channel == MT6370_CHANNEL_CH1) {
		...
		//modify XLLWHLSE-15 by ming.liao 20180124 start
		if (mt6370_en_ch1)
		{
			if (mt6370_is_torch(mt6370_level_ch1))
			{
				mt6370_en_ch1 = MT6370_ENABLE_TORCH;
				mt6370_en_ch2 = MT6370_ENABLE_TORCH;
			}
			else
			{
		...
		//modify XLLWHLSE-15 by ming.liao 20180124 end
	} else if (channel == MT6370_CHANNEL_CH2) {
		//modify X604 flash by ming.liao 20171128 start
		mt6370_en_ch1 = enable;
		if (mt6370_en_ch1){
			if (mt6370_is_torch(mt6370_level_ch1))
				mt6370_en_ch1 = MT6370_ENABLE_TORCH;
		...
		//modify X604 flash by ming.liao 20171128 end
		mt6370_en_ch2 = enable;
		if (mt6370_en_ch2)
		{
	...
	if ((mt6370_en_ch1 != MT6370_NONE) && (mt6370_en_ch2 != MT6370_NONE)) {
		...
		//add X604 flash by ming.liao 20171128 start
		} else if ((mt6370_en_ch1 == MT6370_ENABLE_TORCH) &&
				(mt6370_en_ch2 == MT6370_ENABLE_TORCH)) {
			mt6370_enable();
		//add X604 flash by ming.liao 20171128 end
		} else {
...
{
	/* Move to set driver for saving power */
	mt6370_disable(); //add for 'flashlight not close when phone shutdown' by ming.liao 20180102
	return 0;
}
...
//modify by ming.liao@reallytek.com for X604 20171127 start
...
//9: flash mode
	unsigned int  mode = 0;
	int channel1 = MT6370_CHANNEL_CH1;
	int channel2 = MT6370_CHANNEL_CH2;
//	int level = 0;

	sscanf(buf, "%d",&mode);
	printk("rlk_cam flash_debug torch_level_store mode=%d\n",mode);

	rgt_torch_level = mode;
	if(((mode > 0) && (mode <= 2)) || (mode == 9)) {
		mt6370_set_driver(1);
		mt6370_set_scenario(
			FLASHLIGHT_SCENARIO_CAMERA | FLASHLIGHT_SCENARIO_COUPLE);
	}
	if((mode > 0) && (mode <= 2)) {
		// set torch mode
		mt6370_set_level(channel1, 4);
		mt6370_set_level(channel2, 4);
		mt6370_operate(channel2, MT6370_ENABLE);
		printk("rlk_cam flash_debug main_torch mode=%d\n", mode);
	} else if (mode == 9) {
		// set flash mode
		mt6370_set_level(channel1, 17);
		mt6370_set_level(channel2, 17);
		mt6370_operate(channel2, MT6370_ENABLE);
		printk("rlk_cam flash_debug main_flash mode=%d\n", mode);
	} else if (mode == 10){
		// set torch mode
		mt6370_set_level(channel1, 4);
		mt6370_set_level(channel2, 4);
		mt6370_operate(channel2, MT6370_ENABLE);
		printk("ludesuo main_torch mode=%d\n", mode);
	} else {
		// close led
		mt6370_set_scenario(
			FLASHLIGHT_SCENARIO_FLASHLIGHT | FLASHLIGHT_SCENARIO_COUPLE);
		mt6370_operate(channel2, MT6370_DISABLE);
		mt6370_release();
		printk("rlk_cam flash_debug disable torch mode=%d\n", mode);
	}

   	return size;
}

//static DEVICE_ATTR(torch_level, 0666, torch_level_show, torch_level_store);
static DEVICE_ATTR_RW(sub_torch_level);
// sys/class/torch/torch/
static int __init rgt_torch_level_init(void)
{
	printk("start\n");
	torch_class = class_create(THIS_MODULE, "sub_torch");
	if (IS_ERR(torch_class))
	{
		printk("Unable to create class, err = %d\n", (int)PTR_ERR(torch_class));
		return 0;
	}
	torch_dev = device_create(torch_class, NULL, 0, 0,"sub_torch");
	if(NULL == torch_dev)
	{
	...
	device_create_file(torch_dev, &dev_attr_sub_torch_level);
...
{
	device_remove_file(torch_dev, &dev_attr_sub_torch_level);
...
//modify by ming.liao@reallytek.com for X604 20171127 end
