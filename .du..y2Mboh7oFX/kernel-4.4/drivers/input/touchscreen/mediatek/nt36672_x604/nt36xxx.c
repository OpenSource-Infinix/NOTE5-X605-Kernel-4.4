 ...
 *
 * $Revision: 16487 $
 * $Date: 2017-09-14 10:28:16 +0800 (周四, 14 九月 2017) $
 *
...
uint8_t nvt_leather_mode_status = 0;
uint8_t nvt_tp_status = TP_UNKNOW;
	...
	do
	{
		...
		*/
		if (bTouchIsAwake == 0) {
			...
			ret = GestureIdTranslate(input_id);
			if (ret < 0) {
				NVT_LOG("gesture touch data report failed!!");
			}
#endif
...
#if NVT_DEBUG
				NVT_DEBUG_EN("input_id = %d,(input_x,input_y)=(%d, %d),input_w =[%d]\n", input_id, input_x, input_y, input_w);
#endif
...
#if LEATHER_MODE
void nvt_Holster_func(int leather_status)
{
	uint8_t buf[10] = {0};
	uint8_t ret = -EINVAL;

	switch (leather_status){
	case 1:
		NVT_LOG("Enabled  Holster ++++++++\n");
		if ( (TP_NORMAL == nvt_tp_status) || (TP_UNKNOW == nvt_tp_status) ) {
			NVT_LOG("enter cover mode\n");
			buf[0] = EVENT_MAP_HOST_CMD;
			buf[1] = 0x75;// to set mode
			//set notworking place wide
			buf[2] = 0x00;//x min low 4 bit
			buf[3] = 0x00;//x min high 4 bit
			buf[4] = 0x00;//y min low 4 bit
			buf[5] = 0x00;//y min high 4 bit
			buf[6] = 0x64;//x max low 4 bit
			buf[7] = 0x00;//x max high 4 bit
			buf[8] = 0xF0;//y max low 4 bit
			buf[9] = 0x07;//y max high 4 bit
			ret = CTP_I2C_WRITE(ts->client, I2C_FW_Address, buf, 10);
			if (ret != 1) {
				NVT_ERR("set glove switch:%d, failed:%d\n", leather_status, ret);
			} else {
				nvt_tp_status = TP_LEATHER_MODE;
			}
		}
		break;
	case 0:
		NVT_LOG("Disabled  Holster ++++++++\n");
		if ( (TP_LEATHER_MODE == nvt_tp_status) || (TP_UNKNOW == nvt_tp_status) ) {
			NVT_LOG("exit cover mode\n");
			buf[0] = EVENT_MAP_HOST_CMD;
			buf[1] = 0x76;
...
static ssize_t nvt_leather_mode_write_proc(struct file *file,const char *buffer, size_t count, loff_t *ppos)
{
	int leather_status = 0;
	uint8_t num = 0;
	int normal_config = 0;
...
#if BOOT_UPDATE_FIRMWARE
	if(update_loading == 1){
		NVT_LOG("fw upgrade in process, can't leather_mode_write\n");
		return -EINVAL;
	}
#endif
	...
	{
		NVT_ERR("nvt_leather_mode_write_proc copy from user fail\n");
		return -EFAULT;
	}

	num = sscanf(temp,"%d",&leather_status);
	if (1 == num) {
		NVT_LOG("Recive from user leather_status=%d\n",leather_status);

		nvt_leather_mode_status = leather_status;
		mutex_lock(&hall_mutex);

		NVT_LOG("[leather_write] bTouchIsAwake = %d\n", bTouchIsAwake);
		if(bTouchIsAwake == 0) {
			NVT_LOG("[leather_write] tp is suspend !\n");
			mutex_unlock(&hall_mutex);
			return count;
		}
		nvt_Holster_func(leather_status);
	} else {
		if(bTouchIsAwake == 0) {
			NVT_LOG("[leather_write] tp is suspend !\n");
			mutex_unlock(&hall_mutex);
		...
		nvt_Holster_func(normal_config);

		NVT_LOG("Recive from user nvt_tp_status=%d\n", nvt_tp_status);
		mutex_unlock(&hall_mutex);
		return -EINVAL;
	}
	mutex_unlock(&hall_mutex);
...
static ssize_t nvt_leather_mode_read_proc(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
	int len,ret = -1;
	char *page = NULL;
	char *ptr = NULL;
...
static const struct file_operations nvt_leather_mode_proc_fops = {
	.write = nvt_leather_mode_write_proc,
	.read = nvt_leather_mode_read_proc,
};
#endif
...
static int32_t nvt_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	#if LEATHER_MODE
	struct proc_dir_entry *proc_leather_entry = NULL;
	#endif

	#if BOOT_UPDATE_FIRMWARE
	int boot_mode = 0;
	#endif
//modify XLLSHLSS-4 by qiang.xue 20171213 start
#if ((TOUCH_KEY_NUM > 0) || defined(WAKEUP_GESTURE))
//modify XLLSHLSS-4 by qiang.xue 20171213 end
	int32_t retry = 0;
#endif
	...
	/*add by yongjian.huang for trassion gesture start*/
	input_set_capability(ts->input_dev, EV_KEY, KEY_F13);
	ret = gesture_init_node();
	if (ret) {
		NVT_ERR("create proc node fail");
	}
	/*add by yongjian.huang for trassion gesture end*/
	wake_lock_init(&gestrue_wakelock, WAKE_LOCK_SUSPEND, "poll-wake-lock");
#endif

#if LEATHER_MODE
	proc_leather_entry = proc_create(NVT_PROC_LEATHER_FILE, 0666, NULL, &nvt_leather_mode_proc_fops);
	if (proc_leather_entry == NULL) {
		NVT_ERR("create_proc_entry %s failed\n", NVT_PROC_LEATHER_FILE);
	}
#endif
	...
	NVT_LOG("boot_mode = %d,NORMAL_BOOT = %d\n", boot_mode,NORMAL_BOOT);
	if(boot_mode == NORMAL_BOOT){
		...
		INIT_WORK(&ts->nvt_work, Boot_Update_Firmware);
		// please make sure boot update start after display reset(RESX) sequence
		queue_work(nvt_fwu_wq, &ts->nvt_work);
	}
#endif
...
#if defined(WAKEUP_GESTURE)
	//add XLLSHLSS-4 by qiang.xue 20171205 start
	ctp_gesture_fun_enable = SetDozeStatus();
	//add XLLSHLSS-4 by qiang.xue 20171205 end
...
{
#if LEATHER_MODE
	uint8_t last_nvt_leather_mode_status = 0;
	int normal_config = 0;
#endif
...
#if LEATHER_MODE
	mutex_lock(&hall_mutex);
	nvt_tp_status = TP_UNKNOW;
rept_switch:
	last_nvt_leather_mode_status = nvt_leather_mode_status;
	NVT_LOG("last_nvt_leather_mode_status=%d,nvt_tp_status = %d\n", last_nvt_leather_mode_status, nvt_tp_status);
	if (1 == last_nvt_leather_mode_status) {
		nvt_Holster_func(last_nvt_leather_mode_status);
	} else {
		nvt_Holster_func(normal_config);
	}
	NVT_LOG("resume nvt_tp_status = %d\n", nvt_tp_status);

	NVT_LOG("nvt_leather_mode_status=%d,last_nvt_leather_mode_status= %d\n", nvt_leather_mode_status, last_nvt_leather_mode_status);
	if (nvt_leather_mode_status != last_nvt_leather_mode_status)
		goto rept_switch;
	mutex_unlock(&hall_mutex);
#endif
...
//add by yongjian.huang start
static char final_ctp_firmware_version[16] = "0xd";
...
static ssize_t tp_firmware_version_show(struct device *dev,struct device_attribute *attr,char *buf)
{
	...
	fw_buf[0] = EVENT_MAP_FWINFO;
	fw_buf[1] = 0x00;
	fw_buf[2] = 0x00;
	ret = CTP_I2C_READ(ts->client, I2C_BLDR_Address, fw_buf, 3);
	if (ret < 0) {
...
static ssize_t tp_vendor_show(struct device *dev,struct device_attribute *attr,char *buf)
{
	...
	if ((strcmp(lcm_name, "nt36672_fhdp_dsi_vdo_tcl_auo_x604")) == 0) {
		return sprintf(buf,"%s","TCL_NT36672_00");
	} else if ((strcmp(lcm_name, "nt36672_fhdp_dsi_vdo_djn_jdi_x604")) == 0) {
		return sprintf(buf,"%s","DJN_NT36672_01");
	} else {
