...
//[PPD]add XLLLHBLES-2499 by xingxing.wang on 20180507 start
#define F2FS_RESERVE_BLOCK              20480
//[PPD]add XLLLHBLES-2499 by xingxing.wang on 20180507 end
				 ...
				 struct inode *inode, blkcnt_t *count)
{
	...
	//[PPD]add XLLLHBLES-2499 by xingxing.wang on 20180507 start
	if(uid_gte(current_fsuid(),KUIDT_INIT(10010)) && (!strstr(current->comm,"process.media"))){
		if (unlikely(sbi->total_valid_block_count + F2FS_RESERVE_BLOCK > avail_user_block_count)) {
			sbi->total_valid_block_count -= (block_t)(*count);
			spin_unlock(&sbi->stat_lock);
			percpu_counter_sub(&sbi->alloc_valid_block_count, *count);
			goto enospc;
		}
	} else if (unlikely(sbi->total_valid_block_count > avail_user_block_count)) {
