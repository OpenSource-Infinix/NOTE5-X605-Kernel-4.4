...
//modify flash timing by ming.liao 20180118 start
#define LM3648_WAIT_TIME 3
#define LM3648_RETRY_TIMES 3
//modify flash timing by ming.liao 20180118 end
...
static int lm3648_hwen_state = 0;//add HLSSPUB-8915 by ming.liao 20180302
/* TODO: define register */

/* define mutex and work queue */
static DEFINE_MUTEX(lm3648_mutex);
static struct work_struct lm3648_work;
static DEFINE_MUTEX(lm3648_pinctrl_lock);//modify for HLSSPUB-8265 mutex_lock by wei.miao 20180130
/* define usage count */
...
static int lm3648_flash_write(struct i2c_client *client, u8 reg, u8 val)
{
	...
	//modify flash timing by ming.liao 20180118 start
	int  mLoopCount = 0;
	while( mLoopCount < LM3648_RETRY_TIMES)
	{
	...
	if(mLoopCount >= 3)
	{
		pr_err("[X604][Camera]lm3648 writing failed,mLoopCount retry > 3!!\n");
	}
	//modify flash timing by ming.liao 20180118 end
	return ret;
}
...
static int lm3648_pinctrl_set(int pin, int state)
{
	...
	case LM3648_PINCTRL_PIN_HWEN:
        ...
        {
			mutex_lock(&lm3648_pinctrl_lock);
			pinctrl_select_state(lm3648_pinctrl, lm3648_hwen_low);
			mutex_unlock(&lm3648_pinctrl_lock);
        }
	...
	lm3648_hwen_state = state;//add HLSSPUB-8915 by ming.liao 20180306
	return ret;
}
 ...
 * lm3648 operations
 *****************************************************************************/
 //torch(mA) = (Code * 5.89) + 5.4(mA)     0x13:116.7mA,two mode 233.4mA
static const unsigned char lm3648_torch_level[LM3648_LEVEL_NUM] = {
	0x20, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
//flash (mA) = (Code * 23.44) + 23.4 (mA) //115.6(mA),233(mA),two mode 231.2(mA) 466(mA)
static const unsigned char lm3648_flash_level[LM3648_LEVEL_NUM] = {
	0x04, 0x09, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
static int lm3648_enable(void)
{
	...
	lm3648_pinctrl_set(LM3648_PINCTRL_PIN_HWEN, LM3648_PINCTRL_PINSTATE_HIGH);
	 ...
	 if(m_duty < 1)
	{
		lm3648_flash_write(LM3648_i2c_client, LM3648_REG_TORCH_LEVEL_LED1,((lm3648_torch_level[m_duty]&0x3F)|0x80));
		msleep(LM3648_WAIT_TIME); //modify flash timing by ming.liao 20180118
		lm3648_flash_write(LM3648_i2c_client, LM3648_REG_ENABLE,0x0B );//torch mode
	}
	//add for CAMSLYBJ-41 front flashlight level ming.liao 20180102 start
	//torch(mA) = (Code * 5.89) + 5.4(mA)     0x13:116.7mA,two mode 233.4mA
	else if(m_duty == 30)
	{
		lm3648_flash_write(LM3648_i2c_client, LM3648_REG_TORCH_LEVEL_LED1,((0x22&0x3F)|0x80)); //63*5.89+5.4=376
		msleep(LM3648_WAIT_TIME); //modify flash timing by ming.liao 20180118
		lm3648_flash_write(LM3648_i2c_client, LM3648_REG_ENABLE,0x0B );//torch mode
	}
	else if(m_duty == 29)
	{
		lm3648_flash_write(LM3648_i2c_client, LM3648_REG_TORCH_LEVEL_LED1,((0x13&0x3F)|0x80)); //33*5.89+5.4=199
		msleep(LM3648_WAIT_TIME); //modify flash timing by ming.liao 20180118
		lm3648_flash_write(LM3648_i2c_client, LM3648_REG_ENABLE,0x0B );//torch mode
	}
	else if(m_duty == 28)
	{
		lm3648_flash_write(LM3648_i2c_client, LM3648_REG_TORCH_LEVEL_LED1,((0x08&0x3F)|0x80)); //16*5.89+5.4=99
		msleep(LM3648_WAIT_TIME); //modify flash timing by ming.liao 20180118
		lm3648_flash_write(LM3648_i2c_client, LM3648_REG_ENABLE,0x0B );//torch mode
	}
	//add for CAMSLYBJ-41 front flashlight level ming.liao 20180102 end
	else
	{
		lm3648_flash_write(LM3648_i2c_client, LM3648_REG_FLASH_LEVEL_LED1,((lm3648_flash_level[m_duty]&0x3F)|0x80));
		msleep(LM3648_WAIT_TIME); //modify flash timing by ming.liao 20180118
		lm3648_flash_write(LM3648_i2c_client, LM3648_REG_ENABLE,0x0F );//flash mode
	}
...
{
    printk("ludesuo__lm3648_disable, lm3648_hwen_state:%d \n",lm3648_hwen_state);
	rgt_sub_torch_level = 0;
//add HLSSPUB-8915 by ming.liao 20180302 start
	if(0 == lm3648_hwen_state) {
		printk("ludesuo lm3648_hwen_state:%d \n",lm3648_hwen_state);
		return 0;
	}
//add HLSSPUB-8915 by ming.liao 20180302 end
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	{
		lm3648_flash_write(LM3648_i2c_client, LM3648_REG_TORCH_LEVEL_LED1,((lm3648_torch_level[0]&0x3F)|0x80));
		msleep(LM3648_WAIT_TIME); //modify for HIOSOREL-2202 avoid lm3648_flash_write issue by wei.miao@reallytek.com 20180118
		lm3648_flash_write(LM3648_i2c_client, LM3648_REG_ENABLE,0x0B );//torch mode ,enable led1 led2
		printk("ludesuo sub_torch mode=%d\n", mode);
	}
	else
