...
#include <mt-plat/mtk_boot_common.h> //modify factory camera fail by ming.liao 20171215
...
//add XLLSHLSS-403 long exp by ming.liao 20180120 start
static kal_uint32 shutter_choose = 0;
#define M_SHUTTER_0_5S 		50751
#define M_SHUTTER_1S 		101502
#define M_SHUTTER_1_5S 	    152253
#define M_SHUTTER_2S 		203004
#define M_SHUTTER_4S 		406008
#define M_SHUTTER_6S 		609013
//add XLLSHLSS-403 long exp by ming.liao 20180120 end

//extern bool ov12a10_read_otp_pdaf_data( kal_uint16 addr, BYTE* data, kal_uint32 size);
/*
...
static imgsensor_info_struct imgsensor_info = {
	...
	.checksum_value = 0x2442829d,//test_Pattern_mode
	...
	.pre = {
        ...
        .mipi_data_lp2hs_settle_dc = 90,    //unit , ns , 85 jack  ???
		.max_framerate = 300,
	},
	.cap = {
		.pclk =  108000000,                //jack yan
		.linelength =  1064,
		.framelength = 3346,
		.startx = 0,
		.starty = 0,
		...
		.mipi_data_lp2hs_settle_dc = 90,  //80   30-120
		.max_framerate = 300,
	},
	/*size@15fps, same as capture*/       //to be modify
	.cap1 = {
		.pclk = 108000000,
		.linelength = 1064,
		.framelength = 6692,
    ...
    .margin =8,                //jack yan
    .min_shutter = 4,        //min shutter check
    .max_frame_length = 0x7fff,//max framelength by sensor register's limitation
    .ae_shut_delay_frame = 0,  //check
    .ae_sensor_gain_delay_frame = 0,  //check
    .ae_ispGain_delay_frame = 2,      //jack
    .ihdr_support = 0,
    .ihdr_le_firstline = 0,
    .sensor_mode_num = 5,         //support sensor mode num (not include PIP)

    .cap_delay_frame = 3,         //enter capture delay frame num //default is ok
...
/* Sensor output window information */ //check out
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[10] =
{
	{ 4256, 3168,   0,   0, 4128,   3088,   2064,   1544,   4,    4,  2048, 1536,  0,   0,   2048, 1536}, //Preview OK
	{ 4256, 3168,   0,   0, 4112,   3088,   4112,   3088,  10,    8,  4096, 3072,  0,   0,   4096, 3072}, //capture
	{ 4256, 3168,   0,   0, 4112,   3088,   4112,   3088,  10,    8,  4096, 3072,  0,   0,   4096, 3072}, //normal-video
	{ 4256, 3168,   0, 872, 4096,   2344,   2048,   1172,  708, 128,   640,  480,  0,   0,    640,  480},//hs-video
	{ 4256, 3168,   0, 872, 4096,   2344,   2048,   1172,  388,   8,  1280,  720,  0,   0,   1280,  720},//slim-video
};

/*PD information update*/ //need to modify
...
static void write_shutter(kal_uint16 shutter)
{
	...
	//add XLLSHLSS-403 long exp by ming.liao 20180120 start
	LOG_INF("shutter_choose is = %u\n", shutter_choose);
	if(shutter_choose == M_SHUTTER_0_5S || shutter_choose == M_SHUTTER_1S || shutter_choose == M_SHUTTER_1_5S ||shutter_choose == M_SHUTTER_2S
		|| shutter_choose == M_SHUTTER_4S ||shutter_choose == M_SHUTTER_6S )
	{
	...
	switch (shutter_choose) {
        case M_SHUTTER_0_5S: //50751
		LOG_INF("0.5s shutter =%d, framelength =%d, realtime_fps =%d\n", shutter,imgsensor.frame_length, realtime_fps);
		write_cmos_sensor(0x3c92, 0x00);
		write_cmos_sensor(0x3c93, 0x03);
		write_cmos_sensor(0x3c94, 0x01);
		write_cmos_sensor(0x3c95, 0xe6);
		break;
		case M_SHUTTER_1S: //101502
		LOG_INF("1s shutter =%d, framelength =%d, realtime_fps =%d\n", shutter,imgsensor.frame_length, realtime_fps);
		write_cmos_sensor(0x3c92, 0x00);
		write_cmos_sensor(0x3c93, 0x06);
		write_cmos_sensor(0x3c94, 0x80);
		write_cmos_sensor(0x3c95, 0x65);
		break;
		case M_SHUTTER_1_5S: //152253
		LOG_INF("1.5s shutter =%d, framelength =%d, realtime_fps =%d\n", shutter,imgsensor.frame_length, realtime_fps);
		write_cmos_sensor(0x3c92, 0x00);
		write_cmos_sensor(0x3c93, 0x09);
		write_cmos_sensor(0x3c94, 0x72);
		write_cmos_sensor(0x3c95, 0x1b);
		break;
		case M_SHUTTER_2S: //203004
		LOG_INF("2s shutter =%d, framelength =%d, realtime_fps =%d\n", shutter,imgsensor.frame_length, realtime_fps);
		write_cmos_sensor(0x3c92, 0x00);
		write_cmos_sensor(0x3c93, 0x0c);
		write_cmos_sensor(0x3c94, 0xaa);
		write_cmos_sensor(0x3c95, 0x36);
		break;
		case M_SHUTTER_4S: //406008
		LOG_INF("4s shutter =%d, framelength =%d, realtime_fps =%d\n", shutter,imgsensor.frame_length, realtime_fps);
		write_cmos_sensor(0x3c92, 0x00);
		write_cmos_sensor(0x3c93, 0x19);
		write_cmos_sensor(0x3c94, 0x8a);
		write_cmos_sensor(0x3c95, 0x9f);
		break;
		case M_SHUTTER_6S: //609013
		LOG_INF("6s shutter =%d, framelength =%d, realtime_fps =%d\n", shutter,imgsensor.frame_length, realtime_fps);
		write_cmos_sensor(0x3c92, 0x00);
		write_cmos_sensor(0x3c93, 0x26);
		write_cmos_sensor(0x3c94, 0x6b);
		write_cmos_sensor(0x3c95, 0x09);
		break;
		default:
        LOG_INF("shutter other = %u\n", shutter_choose);
        break;
    }
		...
		write_cmos_sensor(0x3826, 0x0d);
		write_cmos_sensor(0x3827, 0x04);
		write_cmos_sensor(0x401a, 0x40);
		write_cmos_sensor(0x3714, 0x20);
...
static void sensor_init(void) //check out
{
	...
	write_cmos_sensor(0x0302,0x46); //fix XLLSHLSS-694 RF disturb by ming.liao 20180116
	...
	write_cmos_sensor(0x3c90,0x00); //add XLLSHLSS-403 long exp by ming.liao 20180120
...
{
	LOG_INF("capture_setting RES_4096*3072_zsl_30fps/15fps xxx currefps = %d\n", currefps);
	write_cmos_sensor(0x0100,0x00);
...
static void normal_video_setting(kal_uint16 currefps)//jack yan check
{
...
static void hs_video_setting(void) //jack yan check
{
	write_cmos_sensor(0x0100,0x00);
	mdelay(10);
	...
	write_cmos_sensor(0x366b,0xa4);
	write_cmos_sensor(0x366c,0x20);
	write_cmos_sensor(0x366d,0xa4);
	write_cmos_sensor(0x366e,0x20);
	write_cmos_sensor(0x3714,0x28);
	write_cmos_sensor(0x371a,0x3e);
	write_cmos_sensor(0x3737,0x08);
	write_cmos_sensor(0x3739,0x20);
	...
	write_cmos_sensor(0x3810,0x02);
	write_cmos_sensor(0x3811,0xc4);
	write_cmos_sensor(0x3813,0x80);
	write_cmos_sensor(0x3814,0x03);
	write_cmos_sensor(0x3816,0x03);
	write_cmos_sensor(0x3820,0xab);
	write_cmos_sensor(0x4009,0x0d);
...
{
	LOG_INF("hs_video_setting RES_1280x720_30fps\n");
	write_cmos_sensor(0x0100,0x00);
	mdelay(10);
	...
	write_cmos_sensor(0x37e4,0x36);
	write_cmos_sensor(0x37e6,0x08);
	write_cmos_sensor(0x3800,0x00);//0
	write_cmos_sensor(0x3801,0x00);
	write_cmos_sensor(0x3802,0x03);//872
	write_cmos_sensor(0x3803,0x68);
	write_cmos_sensor(0x3804,0x0f);
	write_cmos_sensor(0x3805,0xff);
	write_cmos_sensor(0x3806,0x09);
	...
	write_cmos_sensor(0x380e,0x0D);//3346
	write_cmos_sensor(0x380f,0x12);
	write_cmos_sensor(0x3810,0x01);//388
	write_cmos_sensor(0x3811,0x84);
	write_cmos_sensor(0x3813,0x08);//8
	write_cmos_sensor(0x3814,0x03);
	write_cmos_sensor(0x3816,0x03);
	write_cmos_sensor(0x3820,0xab);
	write_cmos_sensor(0x4009,0x0d);
...
static kal_uint32 return_sensor_id(void) // jack_yan 0x1241
{
...
extern void app_get_main_camera_name(char *back_name);
#endif
//add devinfo by ming.liao 20171214 end
static int flag_cam; //modify XLLSHLSS-370 by ming.liao 20180201
        ...
        do {
            *sensor_id = return_sensor_id();
			...
			if(*sensor_id == 0x1241)
			{
				if(flag_cam == 11){
					*sensor_id = 0x124b;
					printk("ov12a10 yazhou_debug flag_cam = %d , *sensor_id = %x\n",flag_cam,*sensor_id);
				}else{
					*sensor_id = 0x1241;
					printk("ov12a10 yazhou_debug flag_cam = %d , *sensor_id = %x\n",flag_cam,*sensor_id);
				}
            ...
            if (*sensor_id == imgsensor_info.sensor_id) {
				//add devinfo by ming.liao 20171214 start
				#ifdef CONFIG_TRAN_SYSTEM_DEVINFO
				app_get_main_camera_name("ov12a10_12M_ofilm_1");
				#endif
				//add devinfo by ming.liao 20171214 end
                pr_err("ivcam 11 ov12a read sensor id cuckoo## [get_imgsensor_id] sensor id: 0x%x\n", *sensor_id);
                return ERROR_NONE;
            }
            pr_err("ivcam 11 ov12a read sensor id  fail [get_imgsensor_id] id: 0x%x\n",*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 1;
...
static kal_uint32 open(void) //check out
{
    ...
    LOG_INF("preview 2048*1536@30fps,1168Mbps/lane; video 1280*720@120fps,1168Mbps/lane; capture 4096*3072@30fps,1168Mbps/lane\n");
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
			...
			#endif
			printk("OV12a10 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            if (sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("OV12a10 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
            LOG_INF("OV12a10 Read sensor id fail: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
...
static kal_uint32 get_info(MSDK_SCENARIO_ID_ENUM scenario_id,
                      ...
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)//check out
{
	if(scenario_id == 0)
    ...
    sensor_info->PDAF_Support = 1; //temparily closed /*0: NO PDAF, 1: PDAF Raw Data mode, 2:PDAF VC mode*/  need to modify

	//sensor_info->HDR_Support = 0; /*0: NO HDR, 1: iHDR, 2:mvHDR, 3:zHDR*/
    sensor_info->SensorMIPILaneNumber = imgsensor_info.mipi_lane_num;
	//modify factory camera fail by ming.liao 20171215 start
	if(FACTORY_BOOT != get_boot_mode()){
		LOG_INF("[rlk][camera][x604] Normal boot mode retrun 24M\n");
		sensor_info->SensorClockFreq = imgsensor_info.mclk;
	}else{
		LOG_INF("[rlk][camera][x604] Factory boot mode return 12M\n");
		sensor_info->SensorClockFreq = 12;
	}
	//modify factory camera fail by ming.liao 20171215 end
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    switch (feature_id) {
        ...
        case SENSOR_FEATURE_SET_ESHUTTER:
            shutter_choose=*feature_data; //add XLLSHLSS-403 long exp by ming.liao 20180120
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:
		  ...
		  //ov12a10_read_otp_pdaf_data((kal_uint16 //)(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2)));
			break;
			...
			switch (*feature_data) {
				...
				case MSDK_SCENARIO_ID_SLIM_VIDEO:
					*(MUINT32 *)(uintptr_t)(*(feature_data+1)) = 0;
					break;
				case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
					*(MUINT32 *)(uintptr_t)(*(feature_data+1)) = 0; //modify XLLSHLSS-271 by ming.liao 20180103
					break;
				default:
					*(MUINT32 *)(uintptr_t)(*(feature_data+1)) = 0;
					break;
...
UINT32 OV12A10_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	LOG_INF("@@fanfan  OV12A10_MIPI_RAW_SensorInit() \n");
