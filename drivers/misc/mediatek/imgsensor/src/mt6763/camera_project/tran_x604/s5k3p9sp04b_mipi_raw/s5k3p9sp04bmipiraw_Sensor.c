...
#include "s5k3p9sp04bmipiraw_Sensor_setting.h"
#include "s5k3p9sp04bmipiraw_Sensor.h"
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5K3P9SP04B_SENSOR_ID,		//Sensor ID Value: 0x30C8//record sensor id defined in Kd_imgsensor.h
        ...
        do {
            *sensor_id = return_sensor_id();
			...
			if(*sensor_id  == 0x3109)
			{
				if(flag_cam == 11){
					*sensor_id = 0x310d;
					printk("s5k3p9sp04b yazhou_debug flag_cam = %d , *sensor_id = %x\n",flag_cam,*sensor_id);
				}else{
					*sensor_id = 0x310c;
					printk("s5k3p9sp04b  yazhou_debug flag_cam = %d , *sensor_id = %x\n",flag_cam,*sensor_id);
				}
			...
			#endif
			printk("s5k3p9sp04b moduelID read:0x%x\n",moduelID);
			//modify XLLSHLSS-370 by ming.liao 20180201 end
           if ((*sensor_id == imgsensor_info.sensor_id) && (moduelID == 0x01)) {
#ifdef CONFIG_MTK_CAM_CAL
		//read_imx135_otp_mtk_fmt();
#endif
				//add devinfo by ming.liao 20171214 start
				#ifdef CONFIG_TRAN_SYSTEM_DEVINFO
				app_get_sub_camera_name("s5k3p9_16M_SUNNY_1"); //add devinfo by ming.liao 20171214
				#endif
				//add devinfo by ming.liao 20171214 end
...
static kal_uint32 open(void)
{
    ...
    if ((imgsensor_info.sensor_id != sensor_id) || (moduelID != 0x01))
	return ERROR_SENSOR_CONNECT_FAIL;
                             ...
                             UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	/******************** PDAF START >>> *********/
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:
			...
			/* S5K3p9SP04B_read_eeprom((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2))); */
			break;
	/******************** PDAF END   <<< *********/
	default:
	    break;
...
UINT32 S5K3p9SP04B_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc =  &sensor_func;
	return ERROR_NONE;
}	/*	S5K3p9SP04B_MIPI_RAW_SensorInit	*/
