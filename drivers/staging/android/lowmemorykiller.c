...
struct delayed_work lmk_delay_work;

static ssize_t killed_show(struct kobject *kobj, struct kobj_attribute *attr,
	char *buf)
{
	return sprintf(buf, "%d\n", killed_pid);
}

static struct kobj_attribute lmk_pid_attribute =
	__ATTR(pid, 0444, killed_show, NULL);

static struct attribute *lmk_attrs[] = {
	&lmk_pid_attribute.attr,
	NULL,
};
...
static struct kobject *killed_pid_kobj;
static struct kset *lmk_kset;
// [PPD] lmk_boost end @}
...
#define LOWMEM_P_STATE_OTHER	(0x4)
	...
	/* modify by yafang.mao donot use amr to kill process at 18/03/02 start*/
    /*min_score_adj = min(min_score_adj, amr_adj);*/
    /*modify by yafang.mao donot use amr to kill process at 18/03/02 end*/
#endif
...
// [PPD] lmk_boost start @{
		killed_pid = selected->pid;
		if (killed_pid != -1)
			schedule_delayed_work(&lmk_delay_work, HZ/10);
// [PPD] lmk_boost end @}
...
// [PPD] lmk_boost start @{
static void lmk_work_func (struct work_struct *work)
{
	char pid_rec[32];
	char *envp[2] = {pid_rec, NULL};

	sprintf(pid_rec, "killed_pid=%d", killed_pid);
	kobject_uevent_env(killed_pid_kobj, KOBJ_CHANGE, envp);
	lowmem_print(2, "Send killed process ID %d to framework\n", killed_pid);
...
// [PPD] lmk_boost start @{
	lmk_kset = kset_create_and_add("lmk", NULL, kernel_kobj);
	if (!lmk_kset)
		return -ENOMEM;

	killed_pid_kobj = kobject_create();
	if (!killed_pid_kobj)
		return -ENOMEM;

	killed_pid_kobj->kset = lmk_kset;

	retval = kobject_add(killed_pid_kobj, NULL, "killed_pid");
	if (retval)
		goto error;

	retval = sysfs_create_group(killed_pid_kobj, &lmk_attr_group);
	if (retval) {
		pr_err("%s: can't create killed sysfs group\n", __func__);
		goto error;
	}

	kobject_uevent(killed_pid_kobj, KOBJ_ADD);
	INIT_DELAYED_WORK(&lmk_delay_work, lmk_work_func);

	goto out;

error:
	kobject_put(killed_pid_kobj);
out:
	return retval;
// [PPD] lmk_boost end @}
}
