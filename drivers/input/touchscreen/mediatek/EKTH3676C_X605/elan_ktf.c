...
static uint8_t file_fw_data_truly[] = {
	#include "fw_data_truly_v560E_20180604_1.i"
};
static uint8_t file_fw_data_dijin[] = {
	#include "fw_data_dijin_v5631_20180509.i"
};
#endif
#else
...
/* samir.liu 20180427 */
static int CheckRekStatus(struct i2c_client *client);
...
extern struct gpio_chip *chip_gpio;
extern int tran_gpio_get_value(struct gpio_chip *chip,unsigned offset);
#ifdef IAP_PORTION
...
static int __elan_ktf_ts_poll(struct i2c_client *client)
{
    ...
    do {
        ...
        status=tran_gpio_get_value(chip_gpio,GPIO_CTP_EINT_PIN);
        if(status==0) break;
...
//		private_ts->client->addr=((private_ts->client->addr)&I2C_MASK_FLAG)|I2C_DMA_FLAG|I2C_ENEXT_FLAG;
    ...
    case IOCTL_I2C_INT:
        ...
        put_user( tran_gpio_get_value(chip_gpio,GPIO_CTP_EINT_PIN), ip);
        break;
    case IOCTL_RESUME:
...
static int HID_FW_Update_InDriver(void *d)
{
    ...
    disable_irq(touch_irq);       
    power_lock = 1;
	if ((strcmp(lcm_name, "hx8399c_fhdp_dsi_vdo_truly_auo_x605")) == 0)
	{
		ELAN_INFO( "[elan] Update Firmware  file_fw_data_truly.i\n");
		PageNum = (sizeof(file_fw_data_truly)/sizeof(uint8_t)/PageSize);
		touch_debug(DEBUG_ERROR, "[elan] PageNum = %d.\n", PageNum);
		fw_data = file_fw_data_truly;
	}
	if ((strcmp(lcm_name, "otm1911a_fhdp_dsi_vdo_djn_auo_x605")) == 0)
	{
		ELAN_INFO( "[elan] Update Firmware  file_fw_data_dijin.i\n");
		PageNum = (sizeof(file_fw_data_dijin)/sizeof(uint8_t)/PageSize);
		touch_debug(DEBUG_ERROR, "[elan] PageNum = %d.\n", PageNum);
		fw_data = file_fw_data_dijin;
	}
...
static int elan_ktf_ts_calibrate(struct i2c_client *client) {
	...
	do {
		...
		rc = i2c_master_recv(client, buf_recv, 8);
		printk("[elan] %s: Calibration ack: %02x, %02x, %02x, %02x,%02x, %02x, %02x, %02x\n", __func__,
				 buf_recv[0], buf_recv[1], buf_recv[2], buf_recv[3],buf_recv[4], buf_recv[5], buf_recv[6], buf_recv[7]);

		if(buf_recv[4]!=0x66 || buf_recv[5]!=0x66 || buf_recv[6]!=0x66 || buf_recv[7]!=0x66) {
			retry--;
			rek--;
		} else
			rek = 1;
    ...
    if (rek < 0)
	{
		printk("[elan] %s: Calibration fail(%d)\n", __func__, rek);
		return -1;
	}
	printk("[elan] %s: Calibration success(%d)\n", __func__, rek);
...
static int CheckRekStatus(struct i2c_client *client)
{
	int rc = 0;
	uint8_t buf_recv[67] = {0};
	uint8_t CheckRekCmd[37] = {0x04, 0x00, 0x23, 0x00, 0x03, 0x00, 0x04, 0x53, 0xD0, 0x00, 0x01};
	printk("[elan] %s: Write CheckRekCmd !\n", __func__);
	rc = elan_ktf_ts_get_data(client, CheckRekCmd, buf_recv, 37, 67);
    if (rc < 0)
    {
        printk("[elan] : CheckRekStatus fail !\n");
		return -1;
    }else{
		printk("[elan] %s: (CheckRekStatus) %2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x\n", __func__, buf_recv[0], buf_recv[1], buf_recv[2], buf_recv[3] , buf_recv[4], buf_recv[5], buf_recv[6], buf_recv[7]);
		if((buf_recv[6]==0xff)&&(buf_recv[7]==0xff))
		{
			return 1;
		}
		else
...
//add by zhanghui for factory mode touch err 20170320 end
	...
	//	}
		...
		if((buf[10]==0x2)&&(pen_power_flag==0))//电量低于15%
		{
			...
			input_sync(idev);
		}else if((buf[10]==0x0)&&(pen_power_flag==0))//电量低于5%
		{
...
static ssize_t show_gpio_int(struct device *dev, struct device_attribute *attr, char *buf)
{
   ...
   return sprintf(buf, "Int=%d\n",tran_gpio_get_value(chip_gpio,GPIO_CTP_EINT_PIN));
}
...
{   
		...
		else if ((strcmp(lcm_name, "otm1911a_fhdp_dsi_vdo_djn_auo_x605")) == 0)
		{
			return sprintf(buf, "%s\n","DJN_EKTH3676C_01");
        }
		else
                             ...
                             const struct i2c_device_id *id)
{
    ...
    lcm_name = mtkfb_find_lcm_driver();
		...
		{
		  ELAN_INFO( "[elan] Update Firmware  file_fw_data_dijin.i\n");
		  file_fw_data=file_fw_data_dijin;
		  fw_need_update=1;
		}
		else
		{
		  ELAN_INFO( "[elan] firmware is error need not update\n");
		  fw_need_update=0;
		}
        boot_mode = get_boot_mode();
        ELAN_INFO("[elan] boot_mode=%d,NORMAL_BOOT=%d\n",boot_mode,NORMAL_BOOT);
    ...
    if(fw_need_update)
	{
		...
		if ((New_FW_ID == FW_ID)||(FW_ID==0xffff)){  // for firmware auto-upgrade
			if ((New_FW_VER > (FW_VERSION))||(FW_VERSION==0xffff))
			{
				...
				/* samir.liu 20180427 */
				if(New_FW_VER == FW_VERSION&&(rekey_flag==1))
				{
					disable_irq(touch_irq);
					elan_ktf_ts_calibrate(client);
					enable_irq(touch_irq);
					printk("[elan]:rekey\n");
				}
